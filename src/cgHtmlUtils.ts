...
export const addClass = function (elementId: string, cssClass: string): boolean {
  ...
  if (Object.prototype.hasOwnProperty.call(el, 'className')) return false;
  // @ts-ignore
  const currentClass = el.className;
  if (currentClass.split(' ')
    .includes(cssClass)) {
    return true;
  } // cssclass already present
  // @ts-ignore
 ...
 * @param objectName
 */
export const getArrObjectsProperties = function (objectName: Object): string[] {
  const arr: string[] = [];
 ...
 */
export const getParams = function (url: string): Object {
  const params = {};
...
export const getFloatParam = (paramName: keyof Object, defaultValue = 0.0): number => {
  ...
  if (isNullOrUndefined(window.URLSearchParams)) {
    ...
    value = parseFloat(<string>params.get(paramName));
  }
...
export const getIntegerParam = (paramName: keyof Object, defaultValue = 0): number => {
  ...
  if (isNullOrUndefined(window.URLSearchParams)) {
    const params = getParams(uri);
    // @ts-ignore
    value = parseInt(params[paramName], 10);
  } else {
    const params = new URLSearchParams(uri);
    value = parseInt(<string>params.get(paramName), 10);
  }
  if (Number.isNaN(value)) {
    return defaultValue;
  }
...
export const removeDiacritics = (str: string): string => {
  let res = str;
  if (isNullOrUndefined(res.normalize)) {
