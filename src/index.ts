 ...
 * v.3.0.0 : Using bun on 2026-04-08.
 */
// eslint-disable-next-line import/extensions,import/no-unresolved
import { LIB_VERSION, BUILD_TIME } from './version.ts';
 ...
 * build time
 */
export const buildTime = BUILD_TIME;
 ...
 * @param f the object that you want to check
 * @returns true if f is a Function, false in other cases
 */
 ...
 * @param functionName
 * @returns true if functionName exist as a Function, false in other cases
 */
export const functionExist = (functionName: any): boolean => typeof functionName !== 'undefined'
    && functionName !== null
 ...
 */
export const isEmpty = (variable: any): boolean => typeof variable === 'undefined' || variable === null || variable === '';

/**
 * check if the given fieldId is null, undefined or an empty string(zero length)
 * @param fieldId
 * @returns true if fieldId doesn't exist, null, undefined or an empty string, false in other cases
 */
export const isEmptyField = function (fieldId: string) {
    const fieldEl = getEl(fieldId);
    if (isNullOrUndefined(fieldEl)) return true;
    if (Object.prototype.hasOwnProperty.call(fieldEl, 'value')) return true;
    const fieldValue = (fieldEl as HTMLInputElement).value;
    return (
 ...
 * @param elementId
 * @param cssClass
 * @returns true if the cssclass was added successfully, otherwise false
 */
export const addClass = function (elementId: string, cssClass: string): boolean {
 ...
 * @param elementId
 * @param cssClass
 * @returns true if the cssclass was removed successfully, otherwise false
 */
export const delClass = function (elementId: string, cssClass: string): boolean {
    const el = getEl(elementId);
    if (!el) return false;
...
export const eventFire = function (el: HTMLElement, etype: string): void {
    const evObj = new Event(etype);
    el.addEventListener(etype, (e) => {
        // add something log here...
        console.log(`# in Event ${etype} for element ${el}`, e);
    }, false);
...
export const dateFr2Iso = function (strFrDate: string): string {
    if (isEmpty(strFrDate)) {
        return '';
    }
    let y = null;
    let m = null;
    let d = null;
    [d, m, y] = strFrDate.split('-');
    return [y, m, d].join('-');
};
 ...
 */
export const escapeHtml = function (text: string): string {
 ...
 */
export const unescapeHtml = function (text: string): string {
    if (isNullOrUndefined(text)) {
        return text;
    }
    return text
 ...
 * @param text
 */
export const convertEnt = (text: string): string => {
    ...
    // @ts-ignore
    textFiltered = `${text}`.replace(/&#\d+;/gm, (s) => String.fromCharCode(Number(s.match(/\d+/gm)[0])));
    return textFiltered;
};
 ...
 * @param objectName
 */
export const getArrObjectsProperties = (obj: object): string[] => Object.keys(obj);

/**
 * get a string dump of object keys properties and values.
 * @param objectName
 */
export const dumpObject2String = function (objectName: Object): string {
    let objDump = '';
 ...
 * @param image string corresponding to the path of image
 * @param height
 * @param width
 * @param idElement HtmlElement id that will append the img tag as child
 * @returns true if img tag was added successfully, otherwise false
 */
export const addImg = function (
    image: string,
    height: string,
    width: string,
    idElement: string,
): boolean {
    const el = getEl(idElement);
 ...
 * @param id
 * @param parentId
 * @param className the css class to set for the new div
 * @returns true if div tag was added successfully, otherwise false
 */
export const addDiv = function (
    ...
    if (isNullOrUndefined(parentEl)) return false;
    // @ts-ignore
 ...
 * more info : https://css-tricks.com/debouncing-throttling-explained-examples/
 * to see the visual difference: http://demo.nimius.net/debounce_throttle/
 * @param fn callback function to be called
 * @param delay to wait in milliseconds between numerous call
 * @param args the arguments to pass to function fn
 */
export const debounce = function <T extends Function> (fn: T, delay = 20) {
 ...
 * @param threshhold period to wait in milliseconds
 * @param args the arguments to pass to function fn
 */
export const throttle = (fn: Function, threshhold = 250, ...args: any[]) => {
    let last: number;
...
/**
 * Extract URL parameters using the standard URL API.
 * Resilience: Handles relative URLs, malformed strings, and auto-decoding.
 * * @param {string} url - The URL to parse
 * @returns {Record<string, string>} - A clean object of parameters
 */
export const getParams = (url: string): Record<string, string> => {
    const params: Record<string, string> = {};

    try {
        // We use a dummy base 'http://x' to allow parsing relative URLs (e.g., "/path?a=1")
        // without the URL constructor throwing an error.
        ...
        // Pragmatic fallback: if the URL is totally nonsensical, return empty set
        return {};
    }
...
const getRawParam = (name: string): string | null => {
 ...
 * @param name
 * @param def is used to return a valid float value if paramName was not valid
 */
export const getFloatParam = (name: string, def = 0.0): number => {
    const val = parseFloat(getRawParam(name) || '');
 ...
 * @param name
 * @param def is used to return a valid int value if paramName was not valid
 */
export const getIntegerParam = (name: string, def = 0.0): number => {
    const val = parseInt(getRawParam(name) || '');
    return Number.isNaN(val) ? def : val;
};
...
export const removeDiacritics = (str: string): string => {
    ...
    // Check for the existence of .normalize (modern environment check)
    if (typeof res.normalize !== 'function') {
        ...
        for (const entry of defaultDiacriticsRemovalMap) {
            // No more index [i] = No more TS2532 error
            res = res.replace(entry.letters, entry.base);
        }
    } else {
        // Standard modern approach: Decomposition (NFD) + stripping marks
...
export const removePunctuation = (str: string): string => {
    // removed \- minus because it is used in french like in ecole-de-commerce
