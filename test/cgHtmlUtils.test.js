...
describe('cgHtmlUtils DOM Utilities', () => {

  // cleanup body before each test to avoid side effects
  beforeEach(() => {
  ...
  test('cgHtmlUtils.functionExist(myFunctionName) should return true', () => {
    ...
    expect(functionExist(myFunctionName))
      .toBe(true)
  })
  ...
  test('cgHtmlUtils.isEmptyField(myemptyinput) should return true if input as no value', () => {
    ...
    expect(isEmptyField('myinput'))
      .toBe(false);
    expect(isEmptyField('myemptyinput'))
      .toBe(true);
  })
  ...
  test('cgHtmlUtils.addClass(\'mydiv\', red) should add the class red to the div', () => {
    ...
    const myDiv = document.getElementById('mydiv');
    addClass('mydiv', 'red')
    expect(myDiv.className.includes('red'))
      .toBe(true);
  })
